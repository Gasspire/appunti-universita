...
int main(int argc, char  *argv[])
{
    ...
    MPI_Cart_shift(comm_cart, 0,1,&rank_up,&rank_down); //otteniamo i vicini sopra e sotto
    ...
    do{
        ...
        {
            if (rank != 0) {// Solo chi non è il primo processo calcola la prima riga dato che lui può effettivamente calcolarla a prescindere dato che non ha bisogno dello scambio
                u_new[N+i] = (u[i] + u[N+i-1] + u[N+i+1]+u[(2*N)+i]) * 0.25; 
                double tmp_1 = u_new[N+i] - u[N+i];
                differenza += (tmp_1 * tmp_1);
            }
            
            if (rank != num_processi - 1) {// Solo chi non è l'ultimo processo calcola l'ultima riga per le stesse ragioni descritte sopra
