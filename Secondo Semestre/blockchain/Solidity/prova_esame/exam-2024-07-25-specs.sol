...
contract DMICards is DMICardsSpecs, SimplifiedERC721{
  ...
  function createCard() external payable returns (uint256 id){
    ...
    carte[numero_carte].attackScore = uint(keccak256(abi.encode(block.timestamp,block.prevrandao, 1))) % (50 - 10 + 1) + 10;
    carte[numero_carte].defenseScore = uint(keccak256(abi.encode(block.timestamp,block.prevrandao, 2))) % (50 - 10 + 1) + 10;
  ...
  function launchChallenge(address challenged, uint256 cardId) external payable{
    ...
    cooldown_end_challenge[msg.sender] = block.timestamp + (hoursBetweenOp * 1 hours);
