...
Questa prova verrà fatta tramite una prova \textbf{non interattiva}, generando $c = H(R || \dots)$ e, poiché l'hash non è prevedibile, allora ci si può fidare. Inoltre, poiché $S$ viene mostrato, allora quello Zerocoin non è più spendibile poiché presente nella blockchain.
...
\begin{itemize}
    \item \textbf{Setup}: Viene generato $N = pq$ e $w$, che è un elemento non nullo minore di $N$ (la base dell'accumulatore). 
    \item \textbf{Accumulate(params, $(c_1,\dots, c_n)$)}: Consente di creare il \textbf{commitment di un insieme di valori}. Calcoliamo l'accumulatore come $A = w^{c_1 \cdot \dots \cdot c_n} \pmod N$.
    \item \textbf{GenWitness(params, v, $(c_1,\dots, c_n)$)}: Genera un testimone per dimostrare che l'elemento $v$ appartiene all'insieme $V = \{c_1,\dots, c_n\}$. Se $v \in V$, il testimone $W$ si calcola come l'accumulatore del sottoinsieme privo di $v$, ovvero: $W = \text{Accumulate}(\text{params}, V \setminus \{v\}) = w^{\prod_{c_i \neq v} c_i} \pmod N$.
    \item \textbf{Acc.Verify(params, v, W, A)}: Il verificatore calcola $A' = W^v \pmod N$. Se $A == A'$, la prova è valida e l'appartenenza è verificata.  
\end{itemize}
Questa costruzione è sicura perché l'entità che esegue il setup elimina i fattori $p$ e $q$. Di conseguenza, poiché nessuno conosce la fattorizzazione di $N$ (assunzione RSA), l'unico modo per falsificare una prova sarebbe calcolare la radice $v$-esima di $A$ senza conoscere i fattori, il che è computazionalmente impossibile.
