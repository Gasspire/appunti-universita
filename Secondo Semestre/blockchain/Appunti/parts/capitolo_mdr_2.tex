...
\end{itemize}
Questi nodi forniscono il livello massimo di sicurezza e indipendenza, in quanto non necessitano di fidarsi di terze parti per la validazione delle transazioni che li riguardano. Rappresentano la manifestazione concreta del principio \textbf{don't trust, verify} fondamentale in Bitcoin.
...
\begin{itemize}
    ...
    \item \textbf{Chiave pubblica $i$-esima}: $P_i = G^{x_i} = G^y \cdot G^{\text{hash}(k \parallel i)}$
